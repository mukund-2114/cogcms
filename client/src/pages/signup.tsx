...
export default function Signup() {
  ...
  const { mutate, isPending, error } = useMutation({
    mutationFn: async () => {
      const payload = { id: id || crypto.randomUUID(), name: name || undefined, email: email || undefined };
      await apiRequest("POST", "/api/signup", payload);
    },
    onSuccess: async () => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-hero-blue via-primary to-hero-green p-4">
      <Card className="w-full max-w-md bg-white/95">
        <CardContent className="p-6 space-y-4">
          <h1 className="text-2xl font-bold text-center">Create account (Local)</h1>
          <div className="space-y-2">
            <Label htmlFor="id">User ID (optional)</Label>
            <Input id="id" value={id} onChange={(e) => setId(e.target.value)} placeholder="leave blank to auto-generate" />
          </div>
          <div className="space-y-2">
